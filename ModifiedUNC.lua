local passes, fails, undefined, notests = 0, 0, 0, 0
local running = 0
...
print("UNC Environment Check, Modified by normalperson.")
...
task.defer(function()
	...
	print("⏺️ " .. notests .. " tests were not tested")
end)
...
test("getcallingscript", {}, function()
    getcallingscript(game.Players.LocalPlayer.PlayerScripts.RbxCharacterSounds)
end)
...
test("rconsoleinfo", {"consoleclear"}, function()
    rconsoleinfo("test info")
end)

test("rconsolewarn", {"consoleclear"}, function()
    rconsoleinfo("test warn")
end)

test("rconsoleerr", {"consoleclear"}, function()
    rconsoleinfo("test error")
end)
